...
# Maximize Similarity, e.g., pull (拉近) representation of background and background together.
...
# Maximize Similarity, e.g., pull (拉近) representation of background and background together.
class SimMaxLoss_Patch(nn.Module):
    def __init__(self, metric='cos', alpha=0.25, reduction='mean'):
        super(SimMaxLoss_Patch, self).__init__()
        self.metric = metric
        self.alpha = alpha
        self.reduction = reduction

    def forward(self, embedded_cls, embedding_patch):
        """
        :param embedded_fg: [N, C]
        :param embedded_bg: [N, C]
        :return:
        ...
        if self.metric == 'l2':
            ...
            sim = cos_sim(embedded_cls, embedding_patch)
            loss = -torch.log(sim)
            # loss[loss < 0] = 0
            # _, indices = sim.sort(descending=True, dim=1)
            # _, rank = indices.sort(dim=1)
